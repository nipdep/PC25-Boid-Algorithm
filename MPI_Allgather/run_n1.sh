...
for k in "${STRONG_FLOCK_SIZE[@]}"; do
    for j in "${THREAD_COUNT[@]}"; do
        for i in "${NP_COUNT[@]}"; do
            echo "|FILEOUT|MPI_Allgather|STRONG_SCALE|THREAD_COUNT $j|NP_COUNT $i|BOIDS_COUNT $k|"
...
for k in "${WEAK_FLOCK_SIZE[@]}"; do
    for j in "${THREAD_COUNT[@]}"; do
        for i in "${NP_COUNT[@]}"; do
            echo "|FILEOUT|MPI_Allgather|WEAK_SCALE|THREAD_COUNT $j|NP_COUNT $i|BOIDS_COUNT $k|"
